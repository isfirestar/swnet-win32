...
int asio_tcp_accept( packet_t * packet )
{
	...
	if (!NT_SUCCESS(status)) {
		nis_call_ecr("[nshost.packet.asio_tcp_accept] syscall WSAIoctl for WSAID_ACCEPTEX failed,NTSTATUS=0x%08X,link:%I64d", status, ncb_listen->link);
	...
	if ( ncb_income ) {
		if ( !WSAAcceptEx( ncb_listen->sockfd, ncb_income->sockfd, packet->irp_, 0,
			...
			if ( ERROR_IO_PENDING != WSAGetLastError() ) {
				nis_call_ecr("[nshost.packet.asio_tcp_accept] syscall WSAAcceptEx failed,error code=%u,link:%I64d", WSAGetLastError(), ncb_listen->link);
				retval = -1;
			}
		}
...
int asio_tcp_send( packet_t *packet )
{
	...
	if ( retval < 0 ) {
		if ( ERROR_IO_PENDING == WSAGetLastError() ) {
			...
			nis_call_ecr("[nshost.packet.asio_tcp_send] syscall WSASend failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link );
		}
	}
...
int asio_tcp_recv( packet_t * packet )
{
	...
	if ( retval < 0 ) {
		if ( ERROR_IO_PENDING == WSAGetLastError() ) {
			...
			nis_call_ecr("[nshost.packet.asio_tcp_recv] syscall WSARecv failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link );
		}
	}
...
int asio_tcp_connect(packet_t *packet){
	...
	do {
		...
		if (!NT_SUCCESS(status)) {
			nis_call_ecr("[nshost.packet.asio_tcp_connect] syscall WSAIoctl for WSAID_CONNECTEX failed,NTSTATUS=0x%08X,link:%I64d", status, ncb->link);
			break;
		}
			...
			sizeof(struct sockaddr), NULL, 0, NULL, &packet->overlapped_))
		{
			...
			if (ERROR_IO_PENDING != errcode) {
				nis_call_ecr("[nshost.packet.asio_tcp_connect] syscall ConnectEx failed,errcode=%u,link:%ldd", WSAGetLastError(), ncb->link);
				break;
			}
		}
...
int asio_udp_recv( packet_t * packet )
{
	...
	if ( retval < 0 ) {
		if ( ERROR_IO_PENDING == WSAGetLastError() ) {
			...
			nis_call_ecr("[nshost.packet.asio_udp_recv] syscall WSARecvFrom failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link);
		}
	}
...
int syio_udp_send( packet_t * packet, const char *r_ipstr, uint16_t r_port )
{
	...
	if ( retval < 0 ) {
		nis_call_ecr("[nshost.packet.syio_udp_send] syscall WSASendTo failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link);
	}
...
int syio_v_disconnect( ncb_t * ncb )
{
	...
	if ( retval < 0 ) {
		nis_call_ecr("[nshost.packet.syio_v_disconnect] syscall WSAConnect failed,error code=%u,  link:%I64d", WSAGetLastError(), ncb->link);
	}
	return retval;
}
...
int syio_v_connect( ncb_t * ncb, const struct sockaddr_in *r_addr )
{
	...
	if ( retval < 0 ) {
		nis_call_ecr("[nshost.packet.syio_v_connect] syscall WSAConnect failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link);
		return -1;
	}
...
int syio_grp_send( packet_t * packet )
{
	...
	do {
		if ( !WSATransmitPackets ) {
			...
			if ( !NT_SUCCESS( status ) ) {
				nis_call_ecr("[nshost.packet.syio_grp_send] syscall WSAIoctl for GUID_TRANSMIT_PACKETS failed,NTSTATUS=0x%08X, link:%I64d", status, ncb->link);
				break;
			}
		}

		if (!WSATransmitPackets(ncb->sockfd, packet->grp_packets_, packet->grp_packets_cnt_, MAXDWORD, NULL, 0)) {
			nis_call_ecr("[nshost.packet.syio_grp_send] syscall WSATransmitPackets failed,error code=%u, link:%I64d", WSAGetLastError(), ncb->link);
			break;
		}
